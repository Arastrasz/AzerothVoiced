-- AzerothVoiced_CustomQuotes.lua
...
local MODULE_NAME = "AzerothVoiced_CustomQuotes"
...
local function EnsureDB()
  ...
  AzerothVoicedDB.libraries = AzerothVoicedDB.libraries or {}
  AzerothVoicedDB.spellTriggers = AzerothVoicedDB.spellTriggers or {}
end
...
local function NotifyCustomQuotesChanged()
  ...
  if AzerothVoiced_EventBus and AzerothVoiced_EventBus.TriggerEvent then
    LogDebug("Event", "Triggering AZEROTH_VOICED_CUSTOM_UPDATED via EventBus")
  ...
  C_Timer.After(0.1, function()
    if AzerothVoicedAPI and AzerothVoicedAPI.RebuildCaches then
...
local function ExportQuotePack(packName)
  EnsureDB()
  local quotes = AzerothVoicedDB.customQuotes
...
local function ImportQuotePack(importString, merge)
  ...
  for _, quote in ipairs(imported) do
    ...
    for _, existing in ipairs(AzerothVoicedDB.customQuotes) do
    ...
    if not isDuplicate then
      table.insert(AzerothVoicedDB.customQuotes, quote)
      added = added + 1
    end
  end
...
-- UI.lua calls: AzerothVoiced_AddCustomQuote(text, category, sound, spec)
local function AddCustomQuote(textOrTable, category, sound, spec)
  ...
  if type(textOrTable) == "table" then
    -- Called with table: AzerothVoiced_AddCustomQuote({text=..., category=...})
    quote = textOrTable
  else
    -- Called with separate args: AzerothVoiced_AddCustomQuote(text, category, sound, spec)
    quote = {
      text = textOrTable,
      category = category,
      sound = sound,
  ...
  table.insert(AzerothVoicedDB.customQuotes, newQuote)
  local quoteIndex = #AzerothVoicedDB.customQuotes
...
local function DeleteCustomQuote(index)
  ...
  if not index or index < 1 or index > #AzerothVoicedDB.customQuotes then
    return false, "Invalid quote index"
  end
  
  local removed = table.remove(AzerothVoicedDB.customQuotes, index)
...
local function ToggleCustomQuote(index, enabled)
  ...
  AzerothVoicedDB.customQuotes[index].enabled = enabled
...
local function GetCustomQuotes()
  EnsureDB()
  return AzerothVoicedDB.customQuotes
end

local function ClearAllCustomQuotes()
  EnsureDB()
  local count = #AzerothVoicedDB.customQuotes
  AzerothVoicedDB.customQuotes = {}
...
local function AddSpellTrigger(spellID, quoteIndices)
  ...
  AzerothVoicedDB.spellTriggers[spellID] = quoteIndices or {}
...
local function GetSpellTriggers()
  EnsureDB()
  return AzerothVoicedDB.spellTriggers
end
...
function AzerothVoiced_AddCustomQuote(textOrTable, category, sound, spec)
  return AddCustomQuote(textOrTable, category, sound, spec)
end

function AzerothVoiced_DeleteCustomQuote(index)
  return DeleteCustomQuote(index)
end

function AzerothVoiced_ToggleCustomQuote(index, enabled)
  return ToggleCustomQuote(index, enabled)
end

function AzerothVoiced_GetCustomQuotes()
  return GetCustomQuotes()
end

function AzerothVoiced_ClearAllCustomQuotes()
  return ClearAllCustomQuotes()
end

function AzerothVoiced_ExportCustomQuotes(packName)
  return ExportQuotePack(packName)
end

function AzerothVoiced_ImportQuotePack(importString, merge)
  return ImportQuotePack(importString, merge)
end

function AzerothVoiced_AddSpellTrigger(spellID, quoteIndices)
...
------------------------------------------------------------
StaticPopupDialogs["AZEROTHVOICED_CONFIRM_CLEAR_CUSTOM"] = {
  text = "Are you sure you want to delete ALL custom quotes?\n\nThis cannot be undone!",
  button1 = "Yes, Delete All",
  button2 = "Cancel",
  OnAccept = function()
    local count = AzerothVoiced_ClearAllCustomQuotes()
    print("|cffffaa00Azeroth Voiced:|r Cleared " .. count .. " custom quotes.")
  end,
  timeout = 0,
  whileDead = true,
  hideOnEscape = true,
...
------------------------------------------------------------
SLASH_AVCUSTOM1 = "/mqc"
SlashCmdList["AVCUSTOM"] = function(msg)
  local args = {}
  ...
  if cmd == "help" or cmd == "" then
    print("|cffffaa00Azeroth Voiced Custom v" .. ADDON_VERSION .. "|r")
    print("  /avc help - Show this help")
    print("  /avc add <category> <text> - Add a quote")
    print("  /avc list - List all custom quotes")
    print("  /avc delete <index> - Delete a quote")
    print("  /avc toggle <index> - Toggle quote on/off")
    print("  /avc export - Export all quotes")
    print("  /avc import <string> - Import quotes")
    print("  /avc clear - Clear all custom quotes")
    print("  /avc status - Show status and bucket counts")
    print("")
  ...
  elseif cmd == "add" then
    if #args < 3 then
      print("|cffff0000Usage:|r /avc add <category> <text>")
    ...
    if success then
      ...
      print("|cff888888Use '/av test " .. normalizedCat .. "' to test it|r")
    else
  ...
  elseif cmd == "status" then
    EnsureDB()
    local quotes = AzerothVoicedDB.customQuotes
    print("|cffffaa00Azeroth Voiced Custom Status:|r")
    ...
    if AzerothVoicedAPI and AzerothVoicedAPI.GetBucketSummary then
      local summary = AzerothVoicedAPI.GetBucketSummary()
      local parts = {}
  ...
  elseif cmd == "delete" or cmd == "remove" then
    ...
    if not index then
      print("|cffff0000Usage:|r /avc delete <index>")
      return
    end
  ...
  elseif cmd == "toggle" then
    local index = tonumber(args[2])
    if not index then
      print("|cffff0000Usage:|r /avc toggle <index>")
      return
    end
    
    EnsureDB()
    local quote = AzerothVoicedDB.customQuotes[index]
    if not quote then
  ...
  elseif cmd == "import" then
    if #args < 2 then
      print("|cffff0000Usage:|r /avc import <string>")
      return
    end
  ...
  elseif cmd == "clear" then
    ...
    print("|cffff0000Unknown command:|r " .. cmd .. " - Use /avc help")
  end
end
...
local function Initialize()
  ...
  LogDebug("Init", "CustomQuotes module v%s loaded with %d quotes", 
           ADDON_VERSION, #AzerothVoicedDB.customQuotes)
  
  -- Initial cache rebuild to include custom quotes
  C_Timer.After(1, function()
    if #AzerothVoicedDB.customQuotes > 0 then
...
print("|cffffaa00Azeroth Voiced CustomQuotes v" .. ADDON_VERSION .. " loaded|r - /avc help")
