-- AzerothVoiced/Modules/QuotePacks.lua
-- OPTIONAL MODULE: Curated Quote Packs
-- v1.0.0 - Can be removed without breaking Azeroth Voiced
--
-- HOW TO DISABLE: Remove this file or comment out in .toc
-- Azeroth Voiced will work perfectly without it.
...
function QuotePacksModule:IsPackInstalled(packId)
  ...
  AzerothVoicedDB.installedPacks = AzerothVoicedDB.installedPacks or {}
  return AzerothVoicedDB.installedPacks[packId] == true
end
...
function QuotePacksModule:InstallPack(packId)
  ...
  for _, q in ipairs(pack.quotes or {}) do
    local dup = false
    for _, e in ipairs(AzerothVoicedDB.customQuotes) do
      if e.text == q.text and e.category == q.category then dup = true break end
    end
    if not dup then
      table.insert(AzerothVoicedDB.customQuotes, {
  ...
  AzerothVoicedDB.installedPacks[packId] = true
  if AzerothVoiced_EventBus then AzerothVoiced_EventBus:TriggerEvent("AZEROTH_VOICED_CUSTOM_UPDATED") end
...
function QuotePacksModule:UninstallPack(packId)
  ...
  local new, removed = {}, 0
  for _, q in ipairs(AzerothVoicedDB.customQuotes or {}) do
    if q.packId ~= packId then table.insert(new, q) else removed = removed + 1 end
  end
  
  AzerothVoicedDB.customQuotes = new
  AzerothVoicedDB.installedPacks[packId] = nil
  if AzerothVoiced_EventBus then AzerothVoiced_EventBus:TriggerEvent("AZEROTH_VOICED_CUSTOM_UPDATED") end
...
-- Register Global API
------------------------------------------------------------
AzerothVoiced_QuotePacks = QuotePacksModule

function AzerothVoiced_GetAvailablePacks() return QuotePacksModule:GetAvailablePacks() end
function AzerothVoiced_GetPackById(id) return QuotePacksModule:GetPackById(id) end
function AzerothVoiced_IsPackInstalled(id) return QuotePacksModule:IsPackInstalled(id) end
function AzerothVoiced_InstallPack(id) return QuotePacksModule:InstallPack(id) end
function AzerothVoiced_UninstallPack(id) return QuotePacksModule:UninstallPack(id) end

------------------------------------------------------------
-- Slash Command
------------------------------------------------------------
SLASH_AVPACKS1 = "/avpacks"
SlashCmdList["AVPACKS"] = function(msg)
  local cmd, arg = msg:match("^(%S*)%s*(.*)$")
  cmd = (cmd or ""):lower()
  
  if cmd == "" or cmd == "help" then
    print("|cffffaa00Azeroth Voiced Packs v" .. MODULE_VERSION .. "|r")
    print("  /avpacks list | install <id> | uninstall <id> | installed | ui")
  ...
  elseif cmd == "installed" then
    ...
    local c = 0
    for id in pairs(AzerothVoicedDB.installedPacks or {}) do
  ...
  elseif cmd == "ui" then
    if AzerothVoiced_OpenPacksBrowser then AzerothVoiced_OpenPacksBrowser()
    else print("|cffff6666UI not loaded|r") end
  end
end

print("|cffffaa00Azeroth Voiced QuotePacks v" .. MODULE_VERSION .. "|r - /avpacks")
