...
local MODULE_NAME = "MageQuotes_Dracthyr"
...
-- Voice actor mapping (must exist in MageQuotesDB.sound.actors)
local ACTORS = {
  humanoid = "MageA", -- Visage / humanoid form
  draconic = "MageB", -- Dragon / true form
}
...
local function SetActor(actorName)
  ...
  if not targetIndex then
    ...
    if snd.debug then
      print("|cffff0000MageQuotes Dracthyr:|r Actor '" .. actorName .. "' not found in voice actors list")
    end
    return
  end
  ...
  if snd.actorIndex ~= targetIndex then
    ...
    if snd.debug then
      print("|cff88ccffMageQuotes Dracthyr:|r Voice switched → " .. actorName)
    end
    
    -- Trigger event for other modules
    if MageQuotes_EventBus and MageQuotes_EventBus.TriggerEvent then
      MageQuotes_EventBus:TriggerEvent("MAGE_QUOTES_VOICE_CHANGED", actorName)
    end
  end
end
...
-- Event Management
------------------------------------------------------------
local eventFrame = CreateFrame("Frame", "MageQuotesDracthyrFrame")
...
local function Initialize()
  ...
  if not MageQuotesDB then return end
  ...
  for _, actor in ipairs(MageQuotesDB.sound.actors) do
  ...
  if not hasHumanoid or not hasDraconic then
    ...
    print("|cffff8800MageQuotes Dracthyr:|r Voice actors '" .. ACTORS.humanoid .. 
  ...
  if MageQuotes_EventBus and MageQuotes_EventBus.RegisterCallback then
    ...
    MageQuotes_EventBus:RegisterCallback("MAGE_QUOTES_SETTINGS_CHANGED", function()
      LogDebug("Event", "Settings changed, checking voice config")
      -- Re-validate actors after settings change
      local actors = MageQuotesDB.sound and MageQuotesDB.sound.actors or {}
      local found = false
...
-- Slash Command
------------------------------------------------------------
SLASH_MQDRACTHYR1 = "/mqdragon"
SlashCmdList["MQDRACTHYR"] = function(msg)
  msg = (msg or ""):lower():trim()
  
  if not MageQuotesDB then
    print("|cffff0000MageQuotes Dracthyr:|r Core not loaded yet")
    return
  end
  
  if msg == "status" then
    local form = MageQuotes_DracthyrAPI.GetCurrentForm()
    local actor = (MageQuotesDB.sound and MageQuotesDB.sound.actors and 
                   MageQuotesDB.sound.actors[MageQuotesDB.sound.actorIndex]) or "unknown"
    print("|cff88ccffMageQuotes Dracthyr:|r")
  ...
  elseif msg == "refresh" or msg == "update" then
    print("|cff88ccffMageQuotes Dracthyr:|r Forcing voice update...")
    auraCache = {} -- Clear cache
    UpdateDracthyrVoice(true)
    
  elseif msg == "test" then
    print("|cff88ccffMageQuotes Dracthyr:|r Testing voice switch...")
    if not MageQuotesDB.sound or not MageQuotesDB.sound.actors then
      print("|cffff0000  Sound config not initialized|r")
      return
    end
    local current = MageQuotesDB.sound.actorIndex or 1
    local actors = MageQuotesDB.sound.actors
    local next = (current % #actors) + 1
    MageQuotesDB.sound.actorIndex = next
    print("  Switched to: " .. actors[next])
    
  else
    print("|cff88ccffMageQuotes Dracthyr v" .. ADDON_VERSION .. "|r")
    print("  /mqdragon status  - Show current state")
    print("  /mqdragon refresh - Force voice update")
    print("  /mqdragon test    - Test voice switching")
  end
end

if MageQuotesDB and MageQuotesDB.debugMode then
