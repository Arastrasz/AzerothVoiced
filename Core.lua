...
local function LogDebug(category, message, ...)
  ...
  if AzerothVoicedDB.debugPrint then
    print(string.format("|cff00ff00[AV Debug]|r [%s] [%s] %s", timestamp, category, formatted))
  end
end

local function LogError(context, err)
  AzerothVoicedDB.errors = AzerothVoicedDB.errors or {}
  table.insert(AzerothVoicedDB.errors, {
    time = date("%Y-%m-%d %H:%M:%S"),
    context = context,
    error = tostring(err)
  })
  
  if #AzerothVoicedDB.errors > 50 then
    table.remove(AzerothVoicedDB.errors, 1)
  end
end
...
local function CreateLocalBus()
  if AzerothVoicedLib and AzerothVoicedLib.CreateLocalBus then
    return AzerothVoicedLib.CreateLocalBus()
  end
...
if not AzerothVoiced_EventBus then
  AzerothVoiced_EventBus = CreateLocalBus()
end

function Core:TriggerEvent(event, ...)
  if AzerothVoiced_EventBus and AzerothVoiced_EventBus.TriggerEvent then
    AzerothVoiced_EventBus:TriggerEvent(event, ...)
  end
end
...
-- Saved Variables
------------------------------------------------------------
AzerothVoicedDB = AzerothVoicedDB or {
...
AzerothVoicedDB.spec = AzerothVoicedDB.spec or {
  preview = "auto",
}
...
AzerothVoicedDB.majorCD = AzerothVoicedDB.majorCD or {
  enabled     = true,
  spamWindow  = 8,
  spamMax     = 2,
  spamLockout = 6,
...
AzerothVoicedDB.midCombat = AzerothVoicedDB.midCombat or {
  enabled = true,
  min     = 18,
  max     = 34,
  chance  = 0.40,
...
AzerothVoicedDB.categories = AzerothVoicedDB.categories or {}
...
local function pickLocale()
  ...
  if AzerothVoiced_GetUIStrings then
    local _, activeLocale = AzerothVoiced_GetUIStrings()
    if activeLocale and IsSupportedLocale(activeLocale) then
      return activeLocale
    end
  end
  -- Fallback to DB or client locale
  local fromDB = AzerothVoicedDB.locale
...
local function _basenameNoExt(path)
  if AzerothVoicedLib and AzerothVoicedLib.BasenameNoExt then 
    return AzerothVoicedLib.BasenameNoExt(path) 
  end
...
local function _trim(s)
  if AzerothVoicedLib and AzerothVoicedLib.Trim then 
    return AzerothVoicedLib.Trim(s) 
  end
...
local function ValidateLocale(localeCode)
  local locale = AzerothVoicedLocale[localeCode]
  if not locale then 
...
local function AzerothVoiced_GetActiveSpecKey()
  local mode = (AzerothVoicedDB.spec and AzerothVoicedDB.spec.preview) or "auto"
...
local function MajorCdSpamGate()
  local cfg = AzerothVoicedDB.majorCD or {}
...
------------------------------------------------------------
local AzerothVoiced_SpellTriggers = {
  ------------------------------------------------------------
...
local function HandleSpellCast(spellID)
  ...
  local trigger = AzerothVoiced_SpellTriggers[spellID]
...
function AzerothVoiced_SendInnerWhisper(text, spec)
  if not text or text == "" then return end

  local sk = spec or (AzerothVoiced_GetActiveSpecKey and AzerothVoiced_GetActiveSpecKey()) or "arcane"
...
rebuildCaches = function()
  ...
  if not valid then
    LogError("Locale", err)
    print("|cffff0000Azeroth Voiced:|r " .. err)
    ACTIVE_LOCALE = "enUS"
  end
  
  local source = AzerothVoicedLocale or {}
  L = source[ACTIVE_LOCALE] or source["enUS"] or AzerothVoicedLocale.enUS

  AzerothVoiced_Data.sections  = {}
  AzerothVoiced_Data.greetings = {}
  AzerothVoiced_Data.allQuotes = {}
  AzerothVoiced_Data.bySection = { init = {}, kill = {}, surv = {}, mid = {}, vict = {}, rare = {}, greet = {} }
  HeaderByKey = {}

  local activeSpecKey = AzerothVoiced_GetActiveSpecKey()
    ...
    for _, q in ipairs(sec.quotes or {}) do
      ...
      if _isSpecAllowed(obj.spec) then
        ...
        if obj.text and obj.text ~= "" then
          ...
          table.insert(AzerothVoiced_Data.allQuotes, obj)
          if skey and AzerothVoiced_Data.bySection[skey] then
            table.insert(AzerothVoiced_Data.bySection[skey], obj)
          end
        end
      end
    end
    table.insert(AzerothVoiced_Data.sections, normSec)
  end
  ...
  for _, g in ipairs(L.greetings or {}) do
    ...
    if _isSpecAllowed(obj.spec) then
      ...
      if obj.text and obj.text ~= "" then
        obj.custom = false
        table.insert(AzerothVoiced_Data.greetings, obj)
        table.insert(AzerothVoiced_Data.bySection.greet, obj)
      end
    end
  end
  ...
  -- Method 1: Direct access to AzerothVoicedDB.customQuotes
  if AzerothVoicedDB and AzerothVoicedDB.customQuotes then
    for _, cq in ipairs(AzerothVoicedDB.customQuotes) do
      if cq.enabled ~= false and _isSpecAllowed(cq.spec) then
        ...
        if AzerothVoiced_Data.bySection[catKey] then
          table.insert(AzerothVoiced_Data.bySection[catKey], obj)
          ...
          table.insert(AzerothVoiced_Data.bySection.init, obj)
  ...
  if AzerothVoiced_CustomQuotesAPI and AzerothVoiced_CustomQuotesAPI.IsEnabled and 
     AzerothVoiced_CustomQuotesAPI.IsEnabled() and AzerothVoiced_GetCustomQuotes then
  ...
  -- Log summary
  local bucketSummary = {}
  for k, v in pairs(AzerothVoiced_Data.bySection) do
    if #v > 0 then
  ...
  LogDebug("Cache", "Rebuilt: %d sections, %d total quotes (%d custom)", 
    #AzerothVoiced_Data.sections, #AzerothVoiced_Data.allQuotes, customQuoteCount)
  ...
  AzerothVoiced_EventBus:TriggerEvent("AZEROTH_VOICED_LOCALE_CHANGED", ACTIVE_LOCALE)
end

------------------------------------------------------------
-- Public APIs
------------------------------------------------------------
-- NOTE: AzerothVoiced_GetUIStrings() and AzerothVoiced_SetLocale() are now defined
-- in AzerothVoiced_UIStrings.lua for UI translations. The Core only needs to
-- rebuild quote caches when locale changes.

-- Hook into locale changes from UIStrings module
local _originalSetLocale = AzerothVoiced_SetLocale
if _originalSetLocale then
  AzerothVoiced_SetLocale = function(loc)
...
if not AzerothVoiced_GetUIStrings then
...
if not AzerothVoiced_SetLocale then
  function AzerothVoiced_SetLocale(loc)
    if not IsSupportedLocale(loc) then
      print("|cffffd700Azeroth Voiced:|r Unsupported locale:", tostring(loc))
      return
    end
    AzerothVoicedDB.locale = loc
    rebuildCaches()
    print("|cffffd700Azeroth Voiced:|r Locale set to", loc)
  end
end

-- Provide AzerothVoiced_GetUIString if not available from UIStrings
if not AzerothVoiced_GetUIString then
  function AzerothVoiced_GetUIString(key, locale)
    locale = locale or ACTIVE_LOCALE or pickLocale()
    local strings = AzerothVoiced_UIStrings and AzerothVoiced_UIStrings[locale]
    if strings and strings[key] then
      return strings[key]
    end
    -- Fallback to English
    if AzerothVoiced_UIStrings and AzerothVoiced_UIStrings["enUS"] then
      return AzerothVoiced_UIStrings["enUS"][key] or key
    end
    return key
  end
end
...
local function GetOrInitCategoryCfg(header)
  ...
  local cat = AzerothVoicedDB.categories[key]
  if not cat then
    cat = { 
      enabled = true, 
      cooldown = AzerothVoicedDB.quoteCooldown or 10, 
      chance = 100 
    }
    AzerothVoicedDB.categories[key] = cat
  end
  return cat
end
...
local function SendChatMsg(msg, qspec)
  ...
  AzerothVoiced_SendInnerWhisper(msg, qspec)
end
...
local function CurrentActor()
  local snd  = AzerothVoicedDB.sound or {}
  local list = snd.actors or { "VoiceA", "VoiceB" }
...
local function BuildPath_Actor_NoLocale(filename)
  ...
  local base  = "Interface\\AddOns\\" .. (addonName or "AzerothVoiced")
  local actor = CurrentActor()
  local tmpl  = (AzerothVoicedDB.sound and AzerothVoicedDB.sound.templateNoLocale)
...
local function BuildPath_Actor_WithLocale(filename)
  ...
  local tmpl  = (AzerothVoicedDB.sound and AzerothVoicedDB.sound.templateWithLocale)
...
local function PlayEntrySound(entry)
  if not (AzerothVoicedDB.sound and AzerothVoicedDB.sound.enabled) then return end
  if not entry then return end

  local chan  = AzerothVoicedDB.sound.channel or "Master"
  local tried = {}
  ...
  local success, err = pcall(function()
    ...
    if not ok and AzerothVoicedDB.sound.debug then
      print("|cffffd700Azeroth Voiced:|r Couldn't play sound. Tried:")
...
function AzerothVoiced_ResetRuntimeCooldowns()
...
EmitFromBucketKey = function(key)
  if not AzerothVoicedDB.enabled then 
    LogDebug("Emit", "Blocked: addon disabled")
    return false 
  end
  if not AzerothVoiced_Data.bySection then 
  ...
  local globalCD = tonumber(AzerothVoicedDB.quoteCooldown or 0)
  ...
  local bucket = AzerothVoiced_Data.bySection[key]
...
EmitFromBucketKey_NoCD = function(key)
  ...
  local buckets = AzerothVoiced_Data.bySection
  if not buckets then 
...
local function SendGreeting()
  if not AzerothVoicedDB.enabled then return end
  local now = GetTime()
  if (now - lastGreetTime) < 120 then return end
  lastGreetTime = now

  local g = AzerothVoiced_Data.greetings
...
local function RareLoopTick()
  ...
  if AzerothVoicedDB.enabled and AzerothVoiced_Data.bySection then
    local rareBucket = AzerothVoiced_Data.bySection.rare or {}
    if #rareBucket > 0 then
      if math.random() < (AzerothVoicedDB.rareChance or 0.02) then
...
local function MidCombatLoopTick(gen)
  ...
  local cfg = AzerothVoicedDB.midCombat or {}
...
Core:SetScript("OnEvent", function(_, event, ...)
  ...
  elseif event == "PLAYER_SPECIALIZATION_CHANGED" then
    if AzerothVoicedDB.spec and AzerothVoicedDB.spec.preview == "auto" then
      rebuildCaches()
  ...
  elseif event == "UNIT_SPELLCAST_SUCCEEDED" then
    ...
    if unit == "player" and spellID then
      ...
      -- HandleSpellCast checks if the spell is in AzerothVoiced_SpellTriggers
  ...
  elseif event == "UNIT_HEALTH" or event == "UNIT_MAXHEALTH" then
    ...
    local threshold = tonumber(AzerothVoicedDB.survHPpct or 35) or 35
...
------------------------------------------------------------
local iconPath = (AzerothVoicedIcons and AzerothVoicedIcons.main) or "Interface\\Icons\\Spell_Frost_IceStorm"
...
local function InitializeLibraries()
  ...
  if LDB then
    local success, err = pcall(function()
      local dataObj = LDB:NewDataObject("AzerothVoiced", {
        type = "data source",
        text = "Azeroth Voiced",
        icon = iconPath,
        OnClick = function(_, button)
          if InCombatLockdown() then
            print("|cffffd700Azeroth Voiced:|r Cannot open during combat")
            return
          end
          if button == "LeftButton" then
            if AzerothVoicedFrame then
              if AzerothVoicedFrame:IsShown() then
                AzerothVoicedFrame:Hide()
              else
                if AzerothVoicedFrame.ShowAndRefresh then
                  AzerothVoicedFrame:ShowAndRefresh()
                else
                  AzerothVoicedFrame:Show()
                end
              end
            end
          elseif button == "RightButton" then
            AzerothVoicedDB.enabled = not AzerothVoicedDB.enabled
            print("|cffffd700Azeroth Voiced:|r " .. (AzerothVoicedDB.enabled and "Enabled" or "Disabled"))
          end
        end,
        OnTooltipShow = function(tt)
          tt:AddLine("Azeroth Voiced v" .. ADDON_VERSION)
      ...
      if DBIcon then
        DBIcon:Register("AzerothVoiced", dataObj, AzerothVoicedDB.minimap or {})
...
C_Timer.After(1.5, function()
  ...
  if not AzerothVoicedDB.minimap or AzerothVoicedDB.minimap.hide then return end
  ...
  local function CreateMinimapButton()
    ...
    if _G.AzerothVoicedMinimapButton then return end
    
    local minimapButton = CreateFrame("Button", "AzerothVoicedMinimapButton", Minimap)
    ...
    minimapButton:SetScript("OnEnter", function(self)
      ...
      GameTooltip:AddLine("Azeroth Voiced v" .. ADDON_VERSION)
...
-- Slash commands
------------------------------------------------------------
SLASH_AZEROTHVOICED1 = "/av"
SlashCmdList["AZEROTHVOICED"] = function(msg)
  local function _t(s) return (s or ""):gsub("^%s+",""):gsub("%s+$","") end
  msg = _t(msg or "")
  
  if msg == "" then
    print("|cffffd700AV:|r Azeroth Voiced v" .. ADDON_VERSION .. " (Midnight)")
    print("|cffffd700AV:|r /av sound <file>    - test sound")
    print("|cffffd700AV:|r /av actor next      - switch actor")
    print("|cffffd700AV:|r /av debug [on|off]  - toggle debug")
    print("|cffffd700AV:|r /av spec <type>     - set spec filter")
    print("|cffffd700AV:|r /av cdspam ...      - anti-spam settings")
    print("|cffffd700AV:|r /av status          - show status")
    print("|cffffd700AV:|r /av auras           - check defensive auras")
    print("|cffffd700AV:|r /av test <category> - test quote emission")
    print("|cffffd700AV:|r /av rebuild         - rebuild quote caches")
    print("|cff888888AV:|r Note: Chat channels disabled in Midnight (Inner Whisper only)")
    return
  end
  ...
  if cmd == "status" then
    print("|cffffd700AV Status:|r")
    print("  Version:", ADDON_VERSION, "(Midnight Compatible)")
    print("  Enabled:", AzerothVoicedDB.enabled and "Yes" or "No")
    print("  Locale:", ACTIVE_LOCALE or "Unknown")
    print("  Channel: Inner Whisper (Midnight restriction)")
    print("  Spec Filter:", AzerothVoicedDB.spec.preview or "auto")
    print("  In Combat:", inCombat and "Yes" or "No")
    print("  Debug Mode:", AzerothVoicedDB.debugMode and "Yes" or "No")
    print("  Total Quotes:", #(AzerothVoiced_Data.allQuotes or {}))
    -- Show bucket counts
    if AzerothVoiced_Data.bySection then
      local buckets = {}
      for k, v in pairs(AzerothVoiced_Data.bySection) do
        if #v > 0 then
  ...
  elseif cmd == "rebuild" then
    rebuildCaches()
    print("|cffffd700AV:|r Caches rebuilt. Total quotes:", #(AzerothVoiced_Data.allQuotes or {}))
    return
    
  elseif cmd == "test" then
    local category = _t(rest):lower()
    if category == "" then
      print("|cffffd700AV:|r Usage: /av test <category>")
      print("|cffffd700AV:|r Categories: init, kill, surv, vict, rare, mid, greet")
      return
    end
    -- Map common names
    ...
    local bucket = AzerothVoiced_Data.bySection and AzerothVoiced_Data.bySection[category]
    if not bucket then
      print("|cffff0000AV:|r Unknown category:", category)
      return
    end
    
    print("|cffffd700AV:|r Testing category '" .. category .. "' (" .. #bucket .. " quotes)")
    if #bucket > 0 then
      EmitFromBucketKey_NoCD(category)
    else
      print("|cffff0000AV:|r No quotes in this category!")
    end
    return
  ...
  elseif cmd == "sound" and rest and rest ~= "" then
    ...
    local tried = {}
    local ok = _tryCandidates(AzerothVoicedDB.sound.channel or "Master", BuildActorCandidatesFromEntry(entry), tried)
    if ok then
      print("|cffffd700AV:|r Played:", tried[#tried])
    else
      print("|cffff0000AV:|r Failed. Tried:")
      for _, p in ipairs(tried) do print(" - " .. p) end
    end
    
  elseif cmd == "actor" and rest == "next" then
    AzerothVoicedDB.sound.actorIndex = (AzerothVoicedDB.sound.actorIndex or 1) + 1
    local list = AzerothVoicedDB.sound.actors or { "VoiceA", "VoiceB" }
    if AzerothVoicedDB.sound.actorIndex > #list then 
      AzerothVoicedDB.sound.actorIndex = 1 
    end
    print("|cffffd700AV:|r Actor:", CurrentActor())
    
  elseif cmd == "debug" then
    if rest == "on" then
      AzerothVoicedDB.debugMode = true
      AzerothVoicedDB.debugPrint = true
    elseif rest == "off" then
      AzerothVoicedDB.debugMode = false
      AzerothVoicedDB.debugPrint = false
    else
      AzerothVoicedDB.debugMode = not AzerothVoicedDB.debugMode
      AzerothVoicedDB.debugPrint = AzerothVoicedDB.debugMode
    end
    print("|cffffd700AV:|r Debug mode:", AzerothVoicedDB.debugMode and "on" or "off")
    
  elseif cmd == "channel" then
    -- Channel selection disabled in Midnight
    print("|cffffd700AV:|r Channel selection is disabled in WoW Midnight.")
    print("|cffffd700AV:|r SendChatMessage is now a protected Blizzard-only function.")
    print("|cffffd700AV:|r All quotes are displayed via Inner Whisper (local only).")
  ...
  elseif cmd == "spec" and rest and rest ~= "" then
    ...
    if ok then
      AzerothVoicedDB.spec = AzerothVoicedDB.spec or {}
      AzerothVoicedDB.spec.preview = rest
      rebuildCaches()
      print("|cffffd700AV:|r Spec preview:", rest)
    else
      print("|cffff0000AV:|r Unknown spec. Use: auto/all/arcane/fire/frost")
    end

  elseif cmd == "auras" or cmd == "aura" then
    print("|cffffd700AV Defensive Auras:|r")
    for _, spellID in ipairs(SURVIVAL_AURAS) do
    ...
    print("|cffffd700AV:|r Survival spells now trigger on CAST (not just buff)")
  ...
  elseif cmd == "cdspam" then
    ...
    if arg1 == "on" or arg1 == "enable" then
      AzerothVoicedDB.majorCD.enabled = true
      print("|cffffd700AV:|r Major-CD anti-spam: ON")
    elseif arg1 == "off" or arg1 == "disable" then
      AzerothVoicedDB.majorCD.enabled = false
      print("|cffffd700AV:|r Major-CD anti-spam: OFF")
    elseif arg1 == "window" and tonumber(arg2) then
      AzerothVoicedDB.majorCD.spamWindow = tonumber(arg2)
      print("|cffffd700AV:|r Anti-spam window =", AzerothVoicedDB.majorCD.spamWindow, "sec")
    elseif arg1 == "max" and tonumber(arg2) then
      AzerothVoicedDB.majorCD.spamMax = tonumber(arg2)
      print("|cffffd700AV:|r Anti-spam max casts =", AzerothVoicedDB.majorCD.spamMax)
    elseif arg1 == "lockout" and tonumber(arg2) then
      AzerothVoicedDB.majorCD.spamLockout = tonumber(arg2)
      print("|cffffd700AV:|r Anti-spam lockout =", AzerothVoicedDB.majorCD.spamLockout, "sec")
    else
      print("|cffffd700AV:|r /av cdspam on|off|window <sec>|max <n>|lockout <sec>")
    end
    
  else
    print("|cffff0000AV:|r Unknown command. Try /av for help")
  end
end
...
AzerothVoicedAPI = {
  ...
  IsEnabled = function()
    return AzerothVoicedDB.enabled
  end,
  ...
  GetBucket = function(key)
    return AzerothVoiced_Data.bySection and AzerothVoiced_Data.bySection[key] or {}
  end,
...
C_Timer.After(0.5, function()
  if AzerothVoiced_EventBus and AzerothVoiced_EventBus.RegisterCallback then
    AzerothVoiced_EventBus:RegisterCallback("AZEROTH_VOICED_CUSTOM_UPDATED", function()
      LogDebug("Event", "Custom quotes updated, rebuilding caches...")
      rebuildCaches()
    end, Core)
    LogDebug("Init", "Registered listener for AZEROTH_VOICED_CUSTOM_UPDATED")
  end
end)
...
-- Initialize
LogDebug("Init", "Azeroth Voiced v%s (Midnight) loading...", ADDON_VERSION)
print("|cffffd700Azeroth Voiced v" .. ADDON_VERSION .. "|r loaded (Midnight compatible)")
rebuildCaches()
StartRareLoop()
