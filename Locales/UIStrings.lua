...
MageQuotes_UIStrings["enUS"] = {
  ...
  enableAddon = "Enable Mage Quotes",
  cooldowns = "Cooldowns",
  ...
  addonEnabled = "Mage Quotes Enabled",
  addonDisabled = "Mage Quotes Disabled",
...
local function InitLocale()
  ...
  if MageQuotesDB and MageQuotesDB.locale and MageQuotes_UIStrings[MageQuotesDB.locale] then
    activeLocale = MageQuotesDB.locale
    return
  end
  -- Fall back to game locale
  local gameLocale = GetLocale and GetLocale() or "enUS"
  if MageQuotes_UIStrings[gameLocale] then
...
function MageQuotes_GetUIString(key)
  -- Try active locale first
  local str = MageQuotes_UIStrings[activeLocale]
  if str and str[key] then
    return str[key]
  end
  -- Fallback to English
  local enStr = MageQuotes_UIStrings["enUS"]
...
function MageQuotes_GetUIStrings()
  return MageQuotes_UIStrings[activeLocale] or MageQuotes_UIStrings["enUS"],
         activeLocale,
         {"enUS", "ruRU", "deDE", "itIT"}
end
...
MageQuotes_SetUILocale = MageQuotes_SetLocale

-- Get current locale
function MageQuotes_GetCurrentUILocale()
  return activeLocale
end

-- Check if locale is supported
function MageQuotes_IsSupportedLocale(locale)
  return MageQuotes_UIStrings[locale] ~= nil
end
