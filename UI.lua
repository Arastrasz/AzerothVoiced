...
local function SafeGetUIStrings()
  if AzerothVoiced_GetUIStrings then
    local ui, locale, locales = AzerothVoiced_GetUIStrings()
...
local function L(key)
  return AzerothVoiced_GetUIString and AzerothVoiced_GetUIString(key) or key
end
------------------------------------------------------------
-- Color Palette
------------------------------------------------------------
...
local function SafeCall(func, ...)
  ...
  if not success then
    print("|cffff0000Azeroth Voiced UI Error:|r " .. tostring(result))
  end
  return success, result
end

local function GetOrInitCategoryCfg(header)
  if _G.AzerothVoicedCore and _G.AzerothVoicedCore.GetOrInitCategoryCfg then
    return _G.AzerothVoicedCore:GetOrInitCategoryCfg(header)
  end
  AzerothVoicedDB.categories = AzerothVoicedDB.categories or {}
  local t = AzerothVoicedDB.categories[header]
  if not t then
    t = { enabled = true, cooldown = AzerothVoicedDB.quoteCooldown or 10, chance = 100 }
    AzerothVoicedDB.categories[header] = t
  end
  return t
end

local function RebuildCore()
  if _G.AzerothVoicedCore and _G.AzerothVoicedCore.Rebuild then
    SafeCall(_G.AzerothVoicedCore.Rebuild, _G.AzerothVoicedCore)
  else
    if AzerothVoiced_EventBus and AzerothVoiced_EventBus.TriggerEvent then
      AzerothVoiced_EventBus:TriggerEvent("MAGE_QUOTES_SETTINGS_CHANGED")
    end
  end
end
...
-- Main Frame
------------------------------------------------------------
local Frame = CreateFrame("Frame", "AzerothVoicedFrame", UIParent, BackdropTemplateMixin and "BackdropTemplate")
...
titleIcon:SetTexture(AzerothVoicedIcons and AzerothVoicedIcons.main or "Interface\\Icons\\Spell_Frost_IceStorm")
...
local function UpdateStatusBadge()
  if AzerothVoicedDB and AzerothVoicedDB.enabled then
...
local AzerothVoiced_OpenQuoteEditor  -- Quote editor modal function
...
masterEnableSwitch:SetChecked(AzerothVoicedDB and AzerothVoicedDB.enabled or false)
masterEnableSwitch.OnValueChanged = function(self, value)
  AzerothVoicedDB = AzerothVoicedDB or {}
  AzerothVoicedDB.enabled = value
  UpdateStatusBadge()
...
local cdSlider = CreateFrame("Slider", "AzerothVoicedUICDSlider", settingsContent, "OptionsSliderTemplate")
cdSlider:SetPoint("TOPLEFT", cdLabel, "BOTTOMLEFT", 0, -15)
cdSlider:SetSize(400, 20)
cdSlider:SetMinMaxValues(0, 60)
cdSlider:SetValue(AzerothVoicedDB and AzerothVoicedDB.quoteCooldown or 10)
cdSlider:SetValueStep(1)
cdSlider:SetObeyStepOnDrag(true)

_G["AzerothVoicedUICDSliderLow"]:SetText("0s")
_G["AzerothVoicedUICDSliderHigh"]:SetText("60s")
_G["AzerothVoicedUICDSliderText"]:SetText(string.format("%.0fs", cdSlider:GetValue()))

cdSlider:SetScript("OnValueChanged", function(self, value)
  _G["AzerothVoicedUICDSliderText"]:SetText(string.format("%.0fs", value))
  AzerothVoicedDB = AzerothVoicedDB or {}
  AzerothVoicedDB.quoteCooldown = value
  if AzerothVoiced_ResetRuntimeCooldowns then
    AzerothVoiced_ResetRuntimeCooldowns()
  end
end)
...
AzerothVoicedDB = AzerothVoicedDB or {}
AzerothVoicedDB.channel = "INNER_WHISPER"
...
for i, channel in ipairs(channels) do
  ...
  btn:SetScript("OnClick", function(self)
    print(ColorText("Azeroth Voiced:", COLORS.accent) .. " Inner Whisper is the only available channel in WoW Midnight.")
  end)
...
for i, spec in ipairs(specs) do
  ...
  btn:SetScript("OnClick", function(self)
    ...
    AzerothVoicedDB.spec.preview = spec.key
    RebuildCore()
  ...
  if AzerothVoicedDB and AzerothVoicedDB.spec and AzerothVoicedDB.spec.preview == spec.key then
...
for i, locale in ipairs(locales) do
  ...
  btn:SetScript("OnClick", function(self)
    if AzerothVoiced_SetLocale then
      AzerothVoiced_SetLocale(locale)
      RebuildCore()
      ...
      print(ColorText("Azeroth Voiced:", COLORS.accent) .. " Language changed to " .. locale .. ". Type " .. ColorText("/reload", COLORS.primary) .. " for full UI translation.")
    end
  end)
  
  -- Get current active locale dynamically
  local currentLocale = AzerothVoiced_GetCurrentUILocale and AzerothVoiced_GetCurrentUILocale() or activeLocale
...
-- Dropdown menu
local quotesDropdown = CreateFrame("Frame", "AzerothVoicedQuotesFilterDropdown", quotesFilterFrame, BackdropTemplateMixin and "BackdropTemplate")
...
PopulateQuotes = function(filterText)
  ...
  -- Standard quotes (from AzerothVoiced_Data)
  ...
  if showStandard then
    for _, sec in ipairs((AzerothVoiced_Data and AzerothVoiced_Data.sections) or {}) do
  ...
  if showCustom and AzerothVoiced_GetCustomQuotes then
    local customQuotes = AzerothVoiced_GetCustomQuotes()
...
local function RebuildCategoryRows()
  ...
  for _, sec in ipairs((AzerothVoiced_Data and AzerothVoiced_Data.sections) or {}) do
    ...
    cd:SetText(tostring(cfg.cooldown or AzerothVoicedDB.quoteCooldown or 10))
    cd:SetMaxLetters(6)
...
audioSwitch:SetChecked(AzerothVoicedDB and AzerothVoicedDB.sound and AzerothVoicedDB.sound.enabled or false)
audioSwitch.OnValueChanged = function(self, value)
  AzerothVoicedDB = AzerothVoicedDB or {}
  AzerothVoicedDB.sound = AzerothVoicedDB.sound or {}
  AzerothVoicedDB.sound.enabled = value
end
...
for i, channel in ipairs(soundChannels) do
  ...
  btn:SetScript("OnClick", function(self)
    AzerothVoicedDB = AzerothVoicedDB or {}
    AzerothVoicedDB.sound = AzerothVoicedDB.sound or {}
    AzerothVoicedDB.sound.channel = channel
  ...
  if AzerothVoicedDB and AzerothVoicedDB.sound and AzerothVoicedDB.sound.channel == channel then
    btn:SetBackdropColor(COLORS.primary[1], COLORS.primary[2], COLORS.primary[3], 0.5)
  end
...
actorCycleBtn:SetScript("OnClick", function()
  ...
  AzerothVoicedDB.sound.actorIndex = (AzerothVoicedDB.sound.actorIndex % #AzerothVoicedDB.sound.actors) + 1
  local newActor = AzerothVoicedDB.sound.actors[AzerothVoicedDB.sound.actorIndex] or "VoiceA"
  actorCycleBtn.label:SetText("Actor: " .. newActor)
  print(ColorText(L("voiceActorChanged") .. " " .. newActor, COLORS.accent))
end)

if AzerothVoicedDB and AzerothVoicedDB.sound and AzerothVoicedDB.sound.actors then
  local idx = AzerothVoicedDB.sound.actorIndex or 1
  local actor = AzerothVoicedDB.sound.actors[idx] or "VoiceA"
...
testSoundBtn:SetScript("OnClick", function()
  if AzerothVoicedAPI and AzerothVoicedAPI.TriggerManualQuote then
    AzerothVoicedAPI.TriggerManualQuote("init")
...
UpdateStats = function()
  ...
  local totalQuotes = #(AzerothVoiced_Data and AzerothVoiced_Data.allQuotes or {})
...
local modules = {
  {
    name = L("moduleCore"),
    desc = L("coreDesc"),
    check = function() return _G.AzerothVoicedCore ~= nil end,
    version = function() return _G.AzerothVoicedCore and _G.AzerothVoicedCore.VERSION or "N/A" end
  },
  {
    name = L("moduleDracthyr"),
    desc = L("dracthyrDesc"),
    check = function() return _G.AzerothVoiced_DracthyrAPI ~= nil end,
    version = function() 
      return _G.AzerothVoiced_DracthyrAPI and _G.AzerothVoiced_DracthyrAPI.GetVersion and _G.AzerothVoiced_DracthyrAPI.GetVersion() or "N/A" 
    end
  },
  {
    name = L("moduleCustomQuotes") or "Custom Quotes",
    desc = L("customQuotesDesc") or "User-created quotes with import/export support",
    check = function() return _G.AzerothVoiced_CustomQuotesAPI ~= nil end,
    version = function() return _G.AzerothVoiced_CustomQuotesAPI and _G.AzerothVoiced_CustomQuotesAPI.VERSION or "N/A" end
  },
  {
    name = L("moduleAPI"),
    desc = L("apiDesc"),
    check = function() return _G.AzerothVoicedAPI ~= nil end,
    version = function() return _G.AzerothVoicedAPI and _G.AzerothVoicedAPI.VERSION or "N/A" end
  },
  {
    name = L("moduleEventBus"),
    desc = L("eventBusDesc"),
    check = function() return _G.AzerothVoiced_EventBus ~= nil end,
...
debugToggle:SetChecked(AzerothVoicedDB and AzerothVoicedDB.debugMode or false)
...
RefreshDebugLog = function()
  if not AzerothVoicedAPI or not AzerothVoicedAPI.GetDebugLog then
    debugText:SetText(ColorText(L("debugNotAvailable"), COLORS.error))
    debugContent:SetHeight(100)
    return
  end
  
  local log = AzerothVoicedAPI.GetDebugLog()
...
debugToggle.OnValueChanged = function(self, value)
  AzerothVoicedDB = AzerothVoicedDB or {}
  AzerothVoicedDB.debugMode = value
  AzerothVoicedDB.debugPrint = value
  RefreshDebugLog()
end
...
debugUpdateFrame:SetScript("OnUpdate", function(self, elapsed)
  ...
  if self.elapsed >= 2 then
    self.elapsed = 0
    if DebugTab:IsVisible() and AzerothVoicedDB and AzerothVoicedDB.debugMode then
      RefreshDebugLog()
    end
  end
end)
...
browsePacksBtn:SetScript("OnClick", function()
  if AzerothVoiced_OpenPacksBrowser then
    AzerothVoiced_OpenPacksBrowser()
  elseif AzerothVoiced_QuotePacks then
    -- Module loaded but UI not available, use slash command
    print("|cffffaa00Azeroth Voiced:|r Use |cff88bbff/avpacks list|r to see packs")
    print("|cffffaa00Azeroth Voiced:|r Use |cff88bbff/avpacks install <id>|r to install")
  end
end)
...
browsePacksBtn:SetScript("OnEnter", function(self)
  ...
  if not AzerothVoiced_OpenPacksBrowser then
    GameTooltip:AddLine("(UI not loaded - will use /avpacks)", 0.6, 0.6, 0.6)
  end
  GameTooltip:Show()
end)
...
local function CheckPacksModule()
  if AzerothVoiced_QuotePacks or AzerothVoiced_GetAvailablePacks then
    browsePacksBtn:Show()
  end
end
...
RefreshCustomQuoteList = function()
  ...
  local quotes = AzerothVoiced_GetCustomQuotes and AzerothVoiced_GetCustomQuotes() or {}
  ...
  for i, quote in ipairs(quotes) do
    ...
    row:SetScript("OnClick", function(self)
      if AzerothVoiced_OpenQuoteEditor then
        AzerothVoiced_OpenQuoteEditor(self.quoteIndex)
      end
    end)
    ...
    toggleBtn:SetScript("OnClick", function(self)
      if AzerothVoiced_ToggleCustomQuote then
        AzerothVoiced_ToggleCustomQuote(self.quoteIndex, not self.currentState)
    ...
    deleteBtn:SetScript("OnClick", function(self)
      if AzerothVoiced_DeleteCustomQuote then
        AzerothVoiced_DeleteCustomQuote(self.quoteIndex)
        RefreshCustomQuoteList()
      end
    end)
...
addQuoteBtn:SetScript("OnClick", function()
  ...
  if AzerothVoiced_AddCustomQuote then
    local success, msg, quoteIndex = AzerothVoiced_AddCustomQuote(text, selectedCategory, sound, selectedSpec)
    if success then
      ...
      if currentQuoteTriggers.spells and #currentQuoteTriggers.spells > 0 and quoteIndex then
        for _, spell in ipairs(currentQuoteTriggers.spells) do
          if AzerothVoiced_AddSpellTrigger then
            AzerothVoiced_AddSpellTrigger(spell.id, { quoteIndex })
          end
        end
      end
...
exportBtn:SetScript("OnClick", function()
  if AzerothVoiced_ExportCustomQuotes then
    local exportStr, err = AzerothVoiced_ExportCustomQuotes("My Quote Pack")
    if exportStr then
      importFrame:Show()
...
doImportBtn:SetScript("OnClick", function()
  ...
  if importStr and importStr ~= "" then
    if AzerothVoiced_ImportQuotePack then
      local success, msg = AzerothVoiced_ImportQuotePack(importStr, true)
      if success then
...
clearAllBtn:SetScript("OnClick", function()
  StaticPopup_Show("AZEROTHVOICED_CONFIRM_CLEAR_CUSTOM")
end)
...
------------------------------------------------------------
local QuoteEditorModal = CreateFrame("Frame", "AzerothVoicedQuoteEditorModal", UIParent, BackdropTemplateMixin and "BackdropTemplate")
...
AzerothVoiced_OpenQuoteEditor = function(quoteIndex)
  if not quoteIndex then return end
  AzerothVoicedDB = AzerothVoicedDB or {}
  AzerothVoicedDB.customQuotes = AzerothVoicedDB.customQuotes or {}
  
  local quote = AzerothVoicedDB.customQuotes[quoteIndex]
...
editSaveBtn:SetScript("OnClick", function()
  ...
  if not newText or newText == "" then
    print("|cffff6666Azeroth Voiced:|r Quote text cannot be empty!")
    return
  end
  ...
  if AzerothVoicedDB.customQuotes[editingQuoteIndex] then
    local quote = AzerothVoicedDB.customQuotes[editingQuoteIndex]
    quote.text = newText
    ...
    if AzerothVoiced_EventBus and AzerothVoiced_EventBus.TriggerEvent then
      AzerothVoiced_EventBus:TriggerEvent("AZEROTH_VOICED_CUSTOM_UPDATED")
    end
    
    print("|cff00ff00Azeroth Voiced:|r Quote #" .. editingQuoteIndex .. " saved!")
  end
...
editDeleteBtn:SetScript("OnClick", function()
  ...
  if AzerothVoiced_DeleteCustomQuote then
    AzerothVoiced_DeleteCustomQuote(editingQuoteIndex)
    print("|cffff9900Azeroth Voiced:|r Quote deleted!")
  end
...
editExportBtn:SetScript("OnClick", function()
  ...
  local quote = AzerothVoicedDB.customQuotes[editingQuoteIndex]
  if quote then
    local cmd = "/avc add " .. (quote.category or "init") .. " " .. (quote.text or "")
    print("|cffffcc00Azeroth Voiced Export:|r Copy this command:")
...
------------------------------------------------------------
local TriggerPickerModal = CreateFrame("Frame", "AzerothVoicedTriggerPicker", UIParent, BackdropTemplateMixin and "BackdropTemplate")
...
tinsert(UISpecialFrames, "AzerothVoicedTriggerPicker")
...
function AzerothVoiced_OpenTriggerPicker(existingTriggers, callback)
...
C_Timer.After(0.1, function()
  if chooseTriggerBtn then
    chooseTriggerBtn:SetScript("OnClick", function()
      AzerothVoiced_OpenTriggerPicker(currentQuoteTriggers, function(triggers)
...
quickTestBtn:SetScript("OnClick", function()
  if AzerothVoicedAPI and AzerothVoicedAPI.TriggerManualQuote then
    ...
    AzerothVoicedAPI.TriggerManualQuote(random)
  end
end)
...
helpBtn:SetScript("OnClick", function()
  print(ColorText("=== Azeroth Voiced Help ===", COLORS.primary))
  print(ColorText("/mq", COLORS.accent) .. " - Show all commands")
  print(ColorText("/av status", COLORS.accent) .. " - Show addon status")
  print(ColorText("/av debug on/off", COLORS.accent) .. " - Toggle debug mode")
  print(ColorText("/av auras", COLORS.accent) .. " - Check defensive auras")
  print(ColorText("/av channel <type>", COLORS.accent) .. " - Set message channel")
end)
